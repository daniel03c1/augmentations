...
class Trainer:
    ...
    def fit(self, train_dataloader, test_dataloader=None, 
            n_epochs=200, scheduler=None, test_mode=False):
        activation = {}
        ...
        for epoch in range(n_epochs):
            ...
            if test_dataloader:
                test_loss, test_acc = self.eval(test_dataloader, test_mode)
                ...
                print(f'{" "*11}validation | loss: {test_loss.item():.5f}, '
                        f'acc: {test_acc.item():.5f}({self.best_acc:.5f})')
            ...
            if self.agent:
                ...
                danger_prob = torch.mean(action[-1, -self.n_transforms:]) \
                            * n_ops
                ...
                # corr
                actions = actions - actions.mean(0, keepdim=True)
    ...
    def eval(self, dataloader, test_mode=False):
        self.model.eval()
